...
- [2026-08-20] — Two multi-step write sequences in Phase 2 are not atomic: the manual Top 3 override swap (clear old slot, set new slot — two sequential client-side writes) and session-end (three sequential writes: `execution_sessions` update, `tasks.last_touched_at`, `streak_log` upsert). A failure mid-sequence in the override swap is self-healing (an empty slot just falls back to the algorithm's pick). Session-end originally hid a mid-sequence failure entirely (cleared the active session and reported success even if the `tasks`/`streak_log` writes failed) — that silent part is fixed as of the same PR's review pass: a `tasks`/`streak_log` failure now surfaces on `ActiveSessionScreen` and keeps the session open so End Session can be retried. The underlying non-atomicity (still three separate network calls, not one transaction) is still deferred. — Deliberate deferral, not a bug to fix now: acceptable at single-user scale. Revisit both together as a single "harden multi-step writes" pass in Phase 10 (Polish), using Postgres functions (`supabase.rpc(...)`) to wrap each sequence in a real transaction — not two separate fixes, since they're the same underlying pattern. Don't re-flag the non-atomicity itself as an open issue before then.
- [2026-08-20] — CodeRabbit flagged that an open `execution_sessions` row (session started, not yet ended) lived only in React state owned by `HomePage`, so switching to Full List and back — or the "← Back" link on the Active Session Screen — silently orphaned it and let a second session start on the same task. Fixed by lifting `useSession` above the route switch in `App.jsx`, disabling the Full List nav link while a session is active, and removing the "← Back" escape hatch (End Session is now the only way off that screen). — **Remaining gap, not yet fixed:** none of this survives an actual browser refresh — `activeSession` is still plain React state with no DB-backed recovery, so refreshing mid-session still orphans the `execution_sessions` row (`ended_at` stays null forever) and the next Top 3 tap can start a duplicate. Revisit as part of the same Phase 10 "harden multi-step writes" pass — on mount, check for an existing open session (`execution_sessions` where `ended_at is null`) and resume it instead of assuming a clean slate. Don't re-flag the in-app-navigation case (that part's done); this note is specifically about refresh/crash recovery.